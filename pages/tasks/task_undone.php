...
$append = '';
if($db->update('tasks', ['done_date' => new Snabb\Database\Literal('NULL')], 'list_id = ' . $task['list_id'] . ' AND task_id =' . $_GET['id']) !== false){
    $append = 'fb=task_undone_success';
}else{
    $append = 'fb=task_undone_failed';
}
if ($redirect === '')
    reload('?'.$append);
else
